...
####################################
# first year costs random variable #
####################################
#this creates the costs array that has information about the costs of each health state
...
#############################################################
# simulate data for plotting  #QALY over time for both policies #
#############################################################
...
for (k in 1:iteration){
  ...
  tempdata_yes_NOAC <- cost_sampler(
  ...
  temp_yes_NOAC_qaly <- NULL
  temp_yes_NOAC_cost <- NULL

  tempdata_no_NOAC <- cost_sampler(
    rate = healthStates_rates_variation, 
    af_rate = healthStates_rates_AF, 
  ...
  for (s in 1:sim) {
    try(temp_yes_NOAC_qaly[s] <- tempdata_yes_NOAC[[s]][length(tempdata_yes_NOAC[[s]][,4]),5])
    try(temp_no_NOAC_qaly[s] <- tempdata_no_NOAC[[s]][length(tempdata_no_NOAC[[s]][,4]),5])
    try(temp_yes_NOAC_cost[s] <- tempdata_yes_NOAC[[s]][length(tempdata_yes_NOAC[[s]][,4]),6])
    try(temp_no_NOAC_cost[s] <- tempdata_no_NOAC[[s]][length(tempdata_no_NOAC[[s]][,4]),6])
  }

  prob_sensitivity_data_Yes_NOAC <- rbind(prob_sensitivity_data_Yes_NOAC, c(effectCoefficients, qaly = mean(temp_yes_NOAC_qaly)/12, cost = mean(temp_yes_NOAC_cost)))
  prob_sensitivity_data_No_NOAC <- rbind(prob_sensitivity_data_No_NOAC, c( effectCoefficients, qaly = mean(temp_no_NOAC_qaly)/12, cost = mean(temp_no_NOAC_cost)))
  print(sprintf("Done %.0f%%", k/iteration*100))
}

colnames(prob_sensitivity_data_Yes_NOAC) <- c("Stroke", "Bleed", "qaly", "cost")
colnames(prob_sensitivity_data_No_NOAC) <- c("Stroke", "Bleed", "qaly", "cost")
